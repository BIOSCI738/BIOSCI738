...
birds %>%
  ...
  annotate(geom = "text", x = 1, y = 110, 
            label = as.expression(bquote(hat(mu)["Phylloscopus collybita"] ==  ~hat(alpha) == ~.(exp(coefs[1])))), 
            col = "#66C2A5", parse = TRUE) + scale_fill_brewer(type = "div", palette = "Set2") +
   theme(legend.position = "none") +
   annotate(geom = "text", x = 1, y = 125, 
            label = as.expression(bquote(hat(mu)["Turdus_merula"] ==  ~hat(alpha) + hat(beta[1])== ~.(exp(coefs[1] + coefs[2])))), 
            col = "#FC8D62", parse = TRUE)
```
